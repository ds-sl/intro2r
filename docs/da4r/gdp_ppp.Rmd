...
df_gdps_countries |> filter(year >=1990) |> 
  ...
  ggplot(aes(year, gdp_nominal, col = factor(iso2c, levels = ISO2CR10))) + geom_line() + scale_y_log10() +labs(title = "Real GDP of Top 10 Countries", col = "")
```
...
df_gdps_countries |> filter(year >=1990) |> 
  ...
  ggplot(aes(year, gdp_nominal, col = factor(iso2c, levels = ISO2CP10))) + geom_line() + scale_y_log10() +labs(title = "GDP PPP of Top 10 Countries", col = "")
```
...
-   There are many world development indicators related gdp.

```{r}
WDIsearch(string = "gdp", field = "name", short = TRUE, cache = NULL)
```

-   Regions are not available in some countries. We need to be careful to select a list of countries.
