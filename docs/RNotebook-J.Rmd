...
*Knit* で他の形式の出力をするときは、最初から一つ一つコード・チャンクを実行して、その結果が出力されますが、R Notebook の、プレビューは、実際にコード・チャンクに表示されているものだけが、含まれます。したがって、たとえば、エラーがあるコードチャンスも、そのまま表示されます。*Knit* で他の形式の出力をするときは、エラーがあると、出力されず、途中で停止します。
