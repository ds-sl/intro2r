---
title: "はじめてのデータ・サイエンス　2023年6月8日"
author: "c1234567 Suzuki, H. 鈴木寛"
...
`tidyverse` Native では、`str` の代わりに、`glimpse` を使います。少し情報が多いからです。でも、この程度のものなら、ほぼ同じです。ここからわかることは、行（データの数 Number of Observations と言います）が16,758、列（変数の数 Number of Variables と言います）が 13 となります。また、`chr` とか、`int` とか `dbl` （`str` では、num）とありますが、これは、文字データか、数値データか、数値の中でも、整数値か、実数値かがわかるようになっています。この違いは、とても重要です。
...
なども可能です。かっこの中に最初に入るものが直前のものという規則になっており、ここでは、データになっています。
...
`head(2)` は、上から２行という意味です。`tidyverse` では、`slice(1:2)` などで、より複雑な部分を取り出せるようにもなっています。
...
1.  1993年ごろまでは増加しているが、そのあとは、増加減少、さまざま。いくつかのピークの年は、`df_gdp %>% filter(country == "Japan")` の下の Next を押して、見ていくと、確認できます。
2.  最後は、コロナの影響かな。
...
2.  アメリカと中国の、GDP が多いので、それを省いて、グラフを書いてみたい。
...
ヒストグラムで見てみましょう。ヒストグラムについては、Posit Primers のこちらの[ビデオ](https://vimeo.com/221607341)がお勧めです。
...
Boxplot については、Posit Primers の[ビデオ](https://vimeo.com/222358034)がお勧めです。
...
#### Fig 12. Income Level による色分け地図 - 1980
