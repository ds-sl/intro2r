...
4.  リンクの右上の Raw ボタンの右の Copy a raw file からコピーして演習用 R Markdown ファイルを作成（あとで再度解説します）[[Rmd](https://github.com/ds-sl/intro2r/blob/main/docs/ges001/unemployment.Rmd)]
...
-   `ggplot(aes(region, electricity_urban)) + geom_bar()`

-   `ggplot(aes(income, electricity_rural)) + geom_bar()`

-   `pivot_longer(cols = c(var1, var2)) |> ggplot(aes(name, value)) + geom_bar()`
