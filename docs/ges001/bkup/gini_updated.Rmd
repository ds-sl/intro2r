...
```{r}
df_gini_extra |> drop_na(gini) |> filter(year >= 2000) |> 
  arrange(desc(gini)) |> distinct(country, year, gini, region)
```

```{r}
df_gini_extra |> filter(year >= 2000) |> drop_na(gini) |> 
...
```{r}
df_gini_extra |> group_by(year) |> drop_na(gini) |> 
  summarize(n = n()) |>
...
df_gini_extra_long |> filter(country == "Japan") |> 
  drop_na(gini) |> distinct(country, year, gini)
```

```{r}
df_gini_extra_long |> filter(country == "Japan") |> filter(levels %in% c("0-20", "20-40", "40-60", "60-80", "80-100")) |>
  drop_na(gini) |> ggplot(aes(levels, value, fill = levels)) + geom_col() + facet_wrap(~year)# + annotate(paste(gini))
```

```{r}
df_gini_extra_long |> filter(country == "Japan") |> filter(levels %in% c("0-20", "20-40", "40-60", "60-80", "80-100")) |>
  drop_na(gini) |> ggplot(aes(as.character(year), value, fill = levels)) + geom_col(position = "dodge", col = "black", linewidth = 0.1) + labs("Distribution of Wealth in Japan", x="year")
```

```{r}
df_gini_extra_long |> filter(country == "United States") |> filter(levels %in% c("0-20", "20-40", "40-60", "60-80", "80-100")) |> filter(year %in% c(2008, 2010, 2013)) |>
  drop_na(gini) |> ggplot(aes(as.character(year), value, fill = levels)) + geom_col(position = "dodge", col = "black", linewidth = 0.1) + labs("Distribution of Wealth in Japan", x="year")
```

```{r}
df_gini_extra_long |> filter(country == "South Africa") |> filter(levels %in% c("0-20", "20-40", "40-60", "60-80", "80-100")) |> filter(year %in% c(2008, 2010, 2013)) |>
  drop_na(gini) |> ggplot(aes(as.character(year), value, fill = levels)) + geom_col(position = "dodge", col = "black", linewidth = 0.1) + labs("Distribution of Wealth in Japan", x="year")
```
...
#### 国ごとに最も最近の GINI 指数（降順）

```{r}
df_gini_extra |> select(country, year, gini:`90-100`, region, income) |> 
  drop_na(gini) |> group_by(country) |> filter(year == max(year)) |> arrange(desc(gini))
```

```{r}
top30gini <- df_gini_extra |>  
  drop_na(gini) |> group_by(country) |> filter(year == max(year)) |> 
  arrange(desc(gini)) |> head(30) |> pull(country)
df_gini_extra |> drop_na(gini) |> filter(country %in% top30gini) |> 
