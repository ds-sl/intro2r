...
-   \$2.15/日 (\$65.4/月, 785/年), \$3.65（\$111.0/月, \$1,332/年）, \$6.85（\$208.4/月, \$2500/年）
...
利用しない変数もあるので select を使って変数を減らすことも可能ですが、今回は、そのまま使うことにしましょう。減らしたい時は、次のようにします。下の例では、見やすいように、`under_2.15` が NA のものは、削除してあります。
...
df_poverty_rate_long |> 
  ...
  drop_na(value) |> group_by(country) |> filter(year == max(year)) |>
  ggplot(aes(country, value, fill = level)) + geom_col(position = "dodge", col = "black", linewidth = 0.1) + 
  labs(title = "Povert Level Ratio of Countries", subtitle = "Using the most recent data")
```
...
```{r}
df_poverty_rate |> filter(country == "Nigeria") |> drop_na(under_2.15) |>
  ggplot(aes(year, under_2.15)) + geom_line()
```

練習 2 いくつかの国の経年変化

```{r}
df_poverty_rate |> filter(country %in% c("Nigeria", "South Africa", "Kenya", "Lao PDR")) |> 
                            drop_na(under_2.15) |>
