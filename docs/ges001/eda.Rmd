...
また、初期設定では、紙のサイズが、US Letter になっています。印刷するときなど、問題が起きる可能性がありますから、必ず、A4 に修正してください。このような問題をさけるためにも、基本的に、R Markdown（R Notebook はその一形式）で文章を作り、Word に出力する場合には、形式を整えた、参照ファイルを作って、それにそって、出力するようにすることもできます。詳しくは、[[R Markdown クックブック](https://gedevan-aleksizde.github.io/rmarkdown-cookbook/word-template.html)] を参照してください。
...
- Output Option: (Use document default), Show output only, show code and output, 
show nothing (run code), show nothing (don't run code)
