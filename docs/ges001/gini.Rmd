...
![](images/clipboard-3722352739.png)
...
### 練習　Indonesia, El Salvador, Paraguay, Ecuador のいずれかについてグラフを描いてみましょう。
...
**問い：７カ国すべてについて、80-100 と、90-100 の部分だけ、表示できないでしょうか。（難：答えは一番下）**
...
df_gini |> 
  filter(year == 2000) |> drop_na(gini) |>
  ggplot(aes(gini,`80-100`)) + geom_point() + 
  geom_smooth(formula = 'y ~ x', method = "lm")
```
